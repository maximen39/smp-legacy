 ...
 */
@Component
public class SkinFileStorageService extends BaseFileStorageService {

    public SkinFileStorageService(Config config) throws IOException {
        super(Paths.get(String.format(config.textureFolder, TextureType.SKIN.dir())));
    }
    ...
    public void store(InputStream inputStream, String name) throws IOException {
        ...
        if (bufferedImage.getWidth() % 64 != 0 || bufferedImage.getWidth() > 1024 ||
                bufferedImage.getHeight() % 32 != 0 || bufferedImage.getHeight() > 512) {
            throw new FileSystemException(name, null, "Размер скина должен быть между 64x32 и 1024x512!");
        }
